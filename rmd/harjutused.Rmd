...
generate_unique_fraction <- function(
  ...
  repeat {
    num <- sample(min_num:max_num, 1)  
    den <- sample(min_den:max_den, 1)    
    ...
    if (!(frac_str %in% existing_values)
        && num != 0 && abs(num) <= 999
        && den <= 999
        && num / den <= max_value) {
...
generate_angles <- function(
  ...
  if (as_fractions) {
    ...
    if (with_pi) {
      ...
      for (i in 1:n_fractions) {
        ...
        new_frac <- generate_unique_fraction(
          existing_values,
          min_num, 
          max_num,
          min_den, 
  ...
  angles_with_pi <- sapply(angles_with_pi, format_latex)
  angles_without_pi <- sapply(angles_without_pi, format_latex, with_pi = FALSE)
...
tasks <- c(
  ...
  tasks_arc_length_by_central_angle_and_radius,
  tasks_arc_length_by_central_angle_and_diameter,
  tasks_arc_length_by_border_angle_and_radius,
  tasks_arc_length_by_border_angle_and_diameter,
  tasks_sector_area_by_central_angle_and_radius,
  tasks_sector_area_by_central_angle_and_diameter,
  tasks_sector_area_by_border_angle_and_radius,
  tasks_sector_area_by_border_angle_and_diameter
)
...
number_of_tasks_at_level_per_pupil <- 1 # 8
number_of_tasks <- number_of_pupils * number_of_tasks_at_level_per_pupil

for (index_of_task in 1:number_of_tasks) {
